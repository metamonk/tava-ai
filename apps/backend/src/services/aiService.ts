...
// Diarized transcript segment from OpenAI
...
// Full diarized transcript structure
export interface DiarizedTranscript {
  segments: TranscriptSegment[];
  fullText: string; // For backward-compatible prompt usage
}
...
interface SpeakerClassification {
  therapist: string; // "A", "B", etc.
  confidence: 'high' | 'medium' | 'low';
  reasoning: string;
}

async function classifySpeakers(
  rawSegments: Array<{ speaker: string; text: string; start: number; end: number }>
): Promise<SpeakerClassification> {
  // Format transcript for analysis
  const transcriptForAnalysis = rawSegments.map((seg) => `${seg.speaker}: ${seg.text}`).join('\n');
  ...
  if (!content) throw new Error('Empty speaker classification response');

  return JSON.parse(content) as SpeakerClassification;
}
...
export async function transcribeAudio(
  ...
  try {
    ...
    console.log(
      `Speaker classification: ${classification.therapist} is therapist (${classification.confidence} confidence)`
    );
...
export function parseTranscript(transcript: string): DiarizedTranscript | null {
  try {
...
export function formatTranscriptForPrompt(transcript: string): string {
  ...
  return diarized.segments.map((seg) => `[${seg.speaker.toUpperCase()}]: ${seg.text}`).join('\n\n');
}
