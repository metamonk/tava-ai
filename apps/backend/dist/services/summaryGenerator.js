    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (!isRetryable || attempt === maxRetries) {
                throw new Error(`Therapist summary generation failed after ${attempt} attempts: ${err.message}`);
            }
...
async function generateClientSummary(transcript, sessionDate, maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const isRetryable = err.status === 429 || (err.status && err.status >= 500) || err.code === 'ETIMEDOUT';
            if (!isRetryable || attempt === maxRetries) {
                throw new Error(`Client summary generation failed after ${attempt} attempts: ${err.message}`);
            }
            await new Promise((resolve) => setTimeout(resolve, Math.pow(2, attempt - 1) * 1000));
        }
    }
